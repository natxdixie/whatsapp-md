		...
		await axios.request({
			...
			axios({
				...
				const ch = cheerio.load(res.data)
                                 ...
                                 axios.get(`https://s.frangky.me/api?link=${vnowm}`).then( nowm => {
                                 axios.get(`https://s.frangky.me/api?link=${vwm}`).then( wm => {
                                 console.log(nowm.data)
				const result = {
						nowatermark: nowm.data.link,
						watermark: wm.data.link
						}
				resolve(result)
                          })
                          })
